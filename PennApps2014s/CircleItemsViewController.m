// 7:46AM sam moore doesn't care about file comment headers
...
@interface CircleItemsViewController ()

@end

@implementation CircleItemsViewController
...
- (PFQuery *)queryForTable {
    ...
    NSString *objectId = (NSString *) self.circle.objectId;
    [query whereKey:@"circleId" equalTo:objectId];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if([segue.identifier isEqualToString:@"claimItems"]) {
        ...
        PFObject *friendObject;
        
        // SM TODO: Don't bone someone randomly 
        for (PFObject *member in self.circle[@"members"]) {
            if (![member isEqual:[PFUser currentUser]]) {
                friendObject = member;
                break;
            }
        }
        ...
        destinationController.recipient    = friendObject;
